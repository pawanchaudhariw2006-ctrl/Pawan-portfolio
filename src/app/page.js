...
export default function Portfolio() {
  ...
  useEffect(() => {
    ...
    const loadThree = async () => {
      ...
      const particleMat = new THREE.PointsMaterial({
        color: 0x22d3ee, size: 0.05, transparent: true, opacity: 0.4
      });
  ...
  return (
    <div className="bg-black text-white min-h-screen flex flex-col font-sans selection:bg-cyan-500 selection:text-black overflow-hidden relative">

      {/* THREE.JS BACKGROUND */}
      <canvas ref={canvasRef} className="fixed top-0 left-0 w-full h-full -z-10 bg-black" />

      {/* NAVIGATION */}
      <nav className="w-full p-6 md:px-12 flex justify-between items-center z-50">
        ...
        <div className="flex gap-4 md:gap-6 text-[16px] font-bold uppercase tracking-wider items-center whitespace-nowrap">
          <span className="text-white cursor-default whitespace-nowrap border-b-2 border-cyan-500 pb-0.5">Home</span>
      ...
      {/* MAIN CONTENT */}
      <main className="flex-grow flex flex-col justify-center items-center px-8 md:px-24 py-10">
        <motion.div
          variants={containerVariants}
          initial="hidden"
          animate="visible"
          className="flex flex-col md:flex-row items-center gap-16 md:gap-24 max-w-7xl w-full"
        >
          {/* Profile Image */}
          <motion.div variants={itemVariants} className="relative z-10 flex-shrink-0">
            <div className="w-72 h-72 md:w-[400px] md:h-[400px] rounded-full border-2 border-dashed border-cyan-500/40 p-4 flex items-center justify-center"
          ...
          {/* Text Content */}
          <div className="flex flex-col items-center md:items-start text-center md:text-left flex-1 z-10">
            <motion.h1 variants={itemVariants} className="text-6xl md:text-7xl font-black tracking-tight mb-6 leading-none">
            ...
            <motion.div variants={itemVariants} className="mb-8 h-10 flex items-center">
              <span className="text-xl md:text-2xl font-bold uppercase tracking-widest text-neutral-300">
                {typewriter}
            ...
            {/* Tags */}
            <motion.div variants={itemVariants} className="flex flex-wrap justify-center md:justify-start gap-5 mb-10">
              {["Artificial Intelligence","Problem-Solving","Machine Learning","Deep Learning","IoT Systems","Computer Vision"].map((tag) => (
                <motion.span
                  whileHover={{ scale: 1.05, backgroundColor: "rgba(34,211,238,0.15)" }}
                  key={tag}
                  className="px-5 py-2 rounded-full border border-cyan-500/20 bg-cyan-500/5 text-cyan-100 text-[18px] font-bold uppercase tracking-wide cursor-default transition-colors"
                >
                  {tag}
                </motion.span>
              ))}
            ...
            <motion.div variants={itemVariants} className="grid grid-cols-1 md:grid-cols-3 gap-8 w-full">
              {[
                { label: "📍 Location", val: "Ner, Maharashtra, India", color: "text-rose-400", link: "https://maps.app.goo.gl/g7ubX4eeDz4wzbqj9" },
                { label: "💼 Expertise", val: "AI/ML Research", color: "text-orange-400", link: null },
                { label: "📧 Contact", val: "pawanchaudhariw2006@gmail.com", color: "text-blue-400", link: "mailto:pawanchaudhariw2006@gmail.com" }
              ].map((item, i) => (
                <motion.div
                ...
                >
                  ...
                  {item.link ? (
                    <a href={item.link} className="text-neutral-100 text-[15px] font-bold leading-tight break-all hover:text-cyan-400 transition">
                      {item.val}
                    </a>
                  ) : (
      ...
      {/* FOOTER */}
      <footer className="w-full px-10 md:px-24 pt-4 pb-6 flex flex-col gap-6 items-center z-10">
        <motion.div
        ...
        >
          <div className="flex flex-col items-center gap-8">
            <span className="text-xs font-bold uppercase tracking-widest text-neutral-00">Connect with me</span>
            <div className="flex gap-12">
              {[
                ...
                <motion.a
                  ...
                  className="w-14 h-14 rounded-xl flex items-center justify-center hover:border-cyan-500 hover:text-cyan-400 transition-all cursor-pointer"
                  style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', backdropFilter: 'blur(10px)' }}
                >
                  <span className="text-[36px]">{social.icon}</span>
                </motion.a>
              ))}
            </div>
          </div>
          ...
          <div className="flex flex-col items-center gap-8">
            <span className="text-xs font-bold uppercase tracking-widest text-neutral-00">See what I'm doing</span>
            <motion.a
              whileHover={{ scale: 1.1, y: -5 }}
              whileTap={{ scale: 0.9 }}
              ...
              style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', backdropFilter: 'blur(10px)' }}
            >
              <span className="text-[36px]"><FaGithub /></span>
            </motion.a>
          </div>
        </motion.div>
